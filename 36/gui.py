...
class AttendanceGUI:
            ...
            if ret:
                ...
                if remaining > 0:
                    ...
                    if self.detection_counts:
                        ...
                        if self.db.mark_attendance(student_id, count):
                            self.tree.insert('', 'end', values=(
                                student_id,
                                student_name,
                                current_time,
    ...
    def export_attendance(self):
        try:
            ...
            with open(filename, 'w', encoding='utf-8') as f:
                f.write("MSSV, Ho ten, Thoi gian, Do tin cay\n")
                for item in self.tree.get_children():
    ...
    def export_attendance(self):
        try:
            ...
            # Thêm tiêu đề cột
            headers = ["MSSV", "Họ tên", "Thời gian", "Độ tin cậy"]
            ws.append(headers)

            # Thêm dữ liệu từ self.tree
            for item in self.tree.get_children():
                values = self.tree.item(item)['values']
                ws.append(values)
